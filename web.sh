...
if [[ $os = 18.04 || $os = 16.04 ]]; then
  apt-get update && apt-get install make build-essential wget net-tools curl vim supervisor pwgen ntpdate git -y
  echo ""
...
if [[ $os = 20.04 ]]; then
  apt-get update && apt-get install make build-essential wget net-tools curl vim python3-pip python3-wheel python3-setuptools ntpdate git -y
  echo ""
  echo "installing supervisor ... "
  echo ""
  apt-get install python3-pip python3-wheel python3-setuptools -y && pip3 install supervisor
fi
...
server {  
  ...
  ssl_certificate        /root/.acme.sh/www.${domain}/fullchain.cer;
  ssl_certificate_key    /root/.acme.sh/www.${domain}/www.${domain}.key;
